...
if config["RUN_BISMARK"] == False:
    if (
        config["RUN_READ_SORTING"]
        or config["RUN_DMR_ANALYSIS"]
        or config["RUN_DOWNSTREAM"]
    ):
        sys.exit(
            "Your conditional parameters are set incorrectly: one step is stopping the workflow and one downstream step is set to True. Please fix this problem before running ARPEGGIO"
        )
...
TRIM_CORES = (
    max(math.trunc(CORES / 6 - 1), 1)
    if config["IS_PAIRED"]
    else max(math.trunc(CORES / 3 - 1), 1)
)
...
if (
    (n_samples_p1 is None)
    or (n_samples_p2 is None)
    or (n_samples_allo is None)
    or (n_samples_A is None)
    or (n_samples_B is None)
):
    sys.exit(
        "There seems to be missing values in your metadata file, please check if the formatting is correct or some entry is missing"
    )
...
if (not config["POLYPLOID_ONLY"]) and (not config["DIPLOID_ONLY"]):
    if n_samples_p1 < 2:
        sys.exit(
            "There seems to be fewer than two samples for your parent1 species, please have at least two samples in your metadata file or make sure the metadata file has the correct formatting (tab-separated columns)"
        )
    elif n_samples_p2 < 2:
        sys.exit(
            "There seems to be fewer than two samples for your parent2 species, please have at least two samples in your metadata file or make sure the metadata file has the correct formatting (tab-separated columns)"
        )
    elif n_samples_allo < 2:
        sys.exit(
            "There seems to be fewer than two samples for your allopolyploid species, please have at least two samples in your metadata file or make sure the metadata file has the correct formatting (tab-separated columns)"
        )
if config["POLYPLOID_ONLY"]:
    n_samples_A = len(
        samples.name[(samples.origin == "allopolyploid") & (samples.condition == "A")]
    )
    n_samples_B = len(
        samples.name[(samples.origin == "allopolyploid") & (samples.condition == "B")]
    )
    if n_samples_A < 2 or n_samples_B < 2:
        sys.exit(
            "There seems to be fewer than two samples for either of your two conditions, please have at least two samples per condition in your metadata file or make sure the metadata file has the correct formatting (tab-separated columns)"
        )
if config["DIPLOID_ONLY"]:
    n_samples_A = len(
        samples.name[
            ((samples.origin == "parent1") | (samples.origin == "parent2"))
            & (samples.condition == "A")
        ]
    )
    n_samples_B = len(
