...
rule bismark_alignment_SE_control:
	...
	output:
		sample = f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_trimmed_bismark_bt2.bam" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_bismark_bt2.bam",
		report = f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_trimmed_bismark_bt2_SE_report.txt" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_bismark_bt2_SE_report.txt"
	log:
		f"logs/bismark_alignment_SE_{{sample}}_control.log"
	params:
		output = lambda w, output: os.path.split(output.sample)[0],
		control = lambda w, input: os.path.split(os.path.split(os.path.split(input.control)[0])[0])[0],
		prefix = "cc",
...
rule bismark_alignment_PE_control:
	...
	output:
		sample = f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_{str(config['PAIR_1'])}_val_1_bismark_bt2_pe.bam" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_{str(config['PAIR_1'])}_bismark_bt2_pe.bam",
		report = f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_{str(config['PAIR_1'])}_val_1_bismark_bt2_PE_report.txt" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Conversion_efficiency/{{sample}}/cc.{{sample}}_{str(config['PAIR_1'])}_bismark_bt2_PE_report.txt"
	log:
...
rule bam_sorting_p1:
	input:
		p1 = f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_1/1.{{sample}}_{str(config['PAIR_1'])}_val_1_bismark_bt2_pe.deduplicated.bam" if config["IS_PAIRED"] and config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_1/1.{{sample}}_{str(config['PAIR_1'])}_bismark_bt2_pe.deduplicated.bam" if config["IS_PAIRED"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_1/1.{{sample}}_trimmed_bismark_bt2.deduplicated.bam" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_1/1.{{sample}}_bismark_bt2.deduplicated.bam"
	output:
...
rule bam_sorting_p2:
	input:
		p2 = f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_{str(config['PAIR_1'])}_val_1_bismark_bt2_pe.deduplicated.bam" if config["IS_PAIRED"] and config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_{str(config['PAIR_1'])}_bismark_bt2_pe.deduplicated.bam" if config["IS_PAIRED"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_trimmed_bismark_bt2.deduplicated.bam" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_bismark_bt2.deduplicated.bam"
	output:
		o2 = f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_{str(config['PAIR_1'])}_val_1_bismark_bt2_pe.deduplicated_sorted.bam" if config["IS_PAIRED"] and config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_{str(config['PAIR_1'])}_bismark_bt2_pe.deduplicated_sorted.bam" if config["IS_PAIRED"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_trimmed_bismark_bt2.deduplicated_sorted.bam" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_2/2.{{sample}}_bismark_bt2.deduplicated_sorted.bam"
	log:
...
rule bam_sorting_allo:
	input:
		f"{OUTPUT_DIR}read_sorting/{{sample}}/{{sample}}_classified{{one_or_two}}.ref.bam" if config["RUN_READ_SORTING"] and config["IS_PAIRED"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_{str(config['PAIR_1'])}_val_1_bismark_bt2_pe.deduplicated.bam" if config["IS_PAIRED"] and config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_{str(config['PAIR_1'])}_bismark_bt2_pe.deduplicated.bam" if config["IS_PAIRED"] else f"{OUTPUT_DIR}read_sorting/{{sample}}_se/{{sample}}_classified{{one_or_two}}.ref.bam" if config["RUN_READ_SORTING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_trimmed_bismark_bt2.deduplicated.bam" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_bismark_bt2.deduplicated.bam"
	output:
		f"{OUTPUT_DIR}read_sorting/{{sample}}/{{sample}}_classified{{one_or_two}}_sorted.ref.bam" if config["RUN_READ_SORTING"] and config["IS_PAIRED"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_{str(config['PAIR_1'])}_val_1_bismark_bt2_pe.deduplicated_sorted_allo.bam" if config["IS_PAIRED"] and config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_{str(config['PAIR_1'])}_bismark_bt2_pe.deduplicated_sorted_allo.bam" if config["IS_PAIRED"] else f"{OUTPUT_DIR}read_sorting/{{sample}}_se/{{sample}}_classified{{one_or_two}}_sorted.ref.bam" if config["RUN_READ_SORTING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_trimmed_bismark_bt2.deduplicated_sorted_allo.bam" if config["RUN_TRIMMING"] else f"{OUTPUT_DIR}Bismark/deduplication/{{sample}}_{{one_or_two}}/{{one_or_two}}.{{sample}}_bismark_bt2.deduplicated_sorted_allo.bam"
	log:
